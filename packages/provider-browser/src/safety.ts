...
import { IrreversibleActionError, SSRFBlockedError } from '@agentctl/core';
...
export class BrowserSafety {
  ...
  public static validateUrl(
    ...
    allowPrivateNetworks: boolean = false
  ): void {
    let parsed: URL;
    try {
      ...
      for (const pattern of this.PRIVATE_IP_PATTERNS) {
        if (pattern.test(hostname)) {
          throw new SSRFBlockedError(
            targetUrl,
            `Access to loopback or private network (${hostname}) is prohibited.`
          );
        }
      }
    ...
    if (allowedOrigins && allowedOrigins.length > 0) {
      ...
      if (!isAllowed) {
        throw new SSRFBlockedError(
          targetUrl,
          `Origin '${origin}' is not in configured allowedOrigins: ${allowedOrigins.join(', ')}`
        );
      }
    }
  }
