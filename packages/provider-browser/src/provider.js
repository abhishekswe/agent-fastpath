 ...
 */
import { PolicyBlockedError, ProviderUnavailableError } from '@agentctl/core';
...
export class PlaywrightBrowserProvider {
    ...
    async navigate(sessionId, url) {
        ...
        BrowserSafety.validateUrl(url, session.bounds.allowedOrigins, session.bounds.allowPrivateNetworks);
    ...
    async observe(sessionId) {
        ...
        const rawElements = await session.page.evaluate(script);
    ...
    async act(sessionId, action, targetOverride) {
        const session = this.sessionManager.getSession(sessionId);
        if (!session) {
            throw new ProviderUnavailableError(this.id, `Session '${sessionId}' not found or expired`);
        }
        ...
        if (session.bounds.maxSteps && session.stepCount > session.bounds.maxSteps) {
            throw new PolicyBlockedError(`Session step budget exceeded (maxSteps: ${session.bounds.maxSteps})`);
        }
        ...
        switch (action.operation) {
            case 'click':
                if (!selector)
                    throw new PolicyBlockedError('Click operation requires a targetRef');
                await session.page.click(selector, { timeout: 5000 });
                break;
            case 'type':
                if (!selector)
                    throw new PolicyBlockedError('Type operation requires a targetRef');
                await session.page.fill(selector, action.textValue || '', { timeout: 5000 });
                break;
            case 'select':
                if (!selector)
                    throw new PolicyBlockedError('Select operation requires a targetRef');
    ...
    async checkOutcome(sessionId, assertion) {
        const session = this.sessionManager.getSession(sessionId);
        if (!session) {
            throw new ProviderUnavailableError(this.id, `Session '${sessionId}' not found or expired`);
        }
        ...
        const hasMatch = assertion.toLowerCase().split(/\s+/).some((word) => word.length > 3 && content.toLowerCase().includes(word));
        return {
            satisfied: hasMatch,
    ...
    async chooseAction(sessionId, goal) {
