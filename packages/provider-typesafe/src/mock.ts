    ...
    for (const [key, q] of Object.entries(request.questions)) {
      if (q.type === 'choice') {
        ...
        if (/relevance|match|relate/i.test(key)) {
          ...
          if (/bug|fix|defect/i.test(stateStr)) selected = 'bugfix';
          else if (/feat|new|add/i.test(stateStr)) selected = 'feature';
          else if (/doc|readme/i.test(stateStr)) selected = 'docs';
          else if (/test/i.test(stateStr)) selected = 'test';
          else selected = optionKeys[0];
        } else if (/ship_verdict/i.test(key)) {
          if (/fail|error|break/i.test(stateStr)) selected = 'BLOCKED';
          else selected = 'READY_TO_SHIP';
        }
