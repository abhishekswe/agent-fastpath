...
export class CapabilityRouter {
    ...
    async evaluate(input) {
        ...
        if (policy.maxStateSizeBytes && byteSize > policy.maxStateSizeBytes) {
            throw new PolicyBlockedError(`Supplied state size (${byteSize} bytes) exceeds maximum limit (${policy.maxStateSizeBytes} bytes)`);
        }
        // 2. Secret Redaction
        let sanitizedState = rawStateStr;
        let appliedRedactions = [];
        ...
        const detResult = DeterministicEngine.evaluateExact(sanitizedState, input.preset, input.presetParams);
        if (detResult.handled) {
            const latencyMs = Date.now() - startTime;
            const metrics = ResultCompactor.computeMetrics(rawStateStr, { decision: detResult.decision, reasonCode: detResult.reasonCode }, latencyMs, 'deterministic', 'deterministic');
            const output = {
        ...
        if (presetDef?.synthesizeDecision) {
            ...
            if (firstKey) {
                ...
                if ('choice' in firstAns)
                    finalDecision = firstAns.choice;
                else if ('score' in firstAns)
                    finalDecision = firstAns.score;
                else if ('answer' in firstAns)
                    finalDecision = firstAns.answer;
            }
        }
        const latencyMs = Date.now() - startTime;
        const metrics = ResultCompactor.computeMetrics(rawStateStr, { decision: finalDecision, answers: judgmentResult.answers }, latencyMs, this.judgmentProvider.id, gateResult.status === 'escalate' ? 'escalation' : 'semantic_jev');
        const output = {
...
//# sourceMappingURL=capability_router.js.map